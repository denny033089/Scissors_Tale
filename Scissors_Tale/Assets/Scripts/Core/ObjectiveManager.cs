...
public class ObjectiveManager : Singleton<ObjectiveManager>
{
        ...
        if (mapData != null && mapData.objectives != null)
        {
            for (int i = 1; i < ObjectiveCount; i++)
            {
                ...
                if (!string.IsNullOrWhiteSpace(obj.description))
                    ...
                    _descriptions[i] = BuildDescription(_types[i], _maxTurns[i], _maxTags[i], _minTags[i], _minHealthPercent[i],mapData.totalTurnLimit);
            }
        }
        else
        {
    ...
    private static string BuildDescription(Enums.ObjectiveType type, int maxTurns, int maxTags, int minTags, int minHealthPercent,int totalTurnLimit)
    {
        switch (type)
        {
